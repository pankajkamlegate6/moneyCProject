...
public class BarChart extends BarLineChartBase<BarData> {
	...
	protected void drawHighlights() {
		...
		for (int i = 0; i < mIndicesToHightlight.length; i++) {
			...
			if (index < mData.getYValCount() && index >= 0
				...
				float x = index * setCount + dataSetIndex
						+ mData.getGroupSpace() / 2f + mData.getGroupSpace()
						* index;
				float y = e.getVal();
	...
	protected void drawData() {
		...
		for (int i = 0; i < setCount; i++) {
			...
			for (int j = 0; j < dataSet.getEntryCount() * mPhaseX; j++) {
				...
				float x = e.getXIndex() + j * (setCount - 1) + i + space * j
						+ space / 2f;
				float y = e.getVal();
	...
	protected void prepareBar(float x, float y, float barspace) {
		...
		if (mDrawBarShadow) {
			mBarShadow.set(mBarRect.left, mOffsetTop, mBarRect.right,
	...
	protected void drawValues() {
		...
		if (mDrawYValues
			...
			for (int i = 0; i < mData.getDataSetCount(); i++) {
				...
				float[] valuePoints = mTrans.generateTransformedValuesBarChart(
						entries, i, mData, mPhaseY);
				...
				if (!mDrawValuesForWholeStack) {
					...
					for (int j = 0; j < valuePoints.length * mPhaseX; j += 2) {
						...
						drawValue(val, valuePoints[j], valuePoints[j + 1]
	...
	protected float getPositiveYOffset(boolean drawAboveValueBar) {
		return (mDrawValueAboveBar ? -Utils.convertDpToPixel(5) : Utils
				.calcTextHeight(mValuePaint, "8") * 1.5f);
	}

	protected float getNegativeYOffset(boolean drawAboveValueBar) {
		return (mDrawValueAboveBar ? Utils.calcTextHeight(mValuePaint, "8") * 1.5f
				: -Utils.convertDpToPixel(5));
	}
