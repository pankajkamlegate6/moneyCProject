...
import com.moneycontrol.handheld.chart.data.BarLineScatterCandleData;
import com.moneycontrol.handheld.chart.data.BarLineScatterCandleRadarDataSet;
import com.moneycontrol.handheld.chart.data.Entry;
import com.moneycontrol.handheld.chart.data.filter.Approximator;
import com.moneycontrol.handheld.chart.interfaces.OnDrawListener;
import com.moneycontrol.handheld.chart.listener.BarLineChartTouchListener;
import com.moneycontrol.handheld.chart.utils.Highlight;
import com.moneycontrol.handheld.chart.utils.Legend.LegendPosition;
import com.moneycontrol.handheld.chart.utils.LimitLine;
import com.moneycontrol.handheld.chart.utils.LimitLine.LimitLabelPosition;
import com.moneycontrol.handheld.chart.utils.PointD;
import com.moneycontrol.handheld.chart.utils.SelInfo;
import com.moneycontrol.handheld.chart.utils.Utils;
import com.moneycontrol.handheld.chart.utils.XLabels;
import com.moneycontrol.handheld.chart.utils.XLabels.XLabelPosition;
import com.moneycontrol.handheld.chart.utils.YLabels;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleRadarDataSet<? extends Entry>>>
	...
	int highImagePosition = 0, lowImagePosition = 0;
	float highImageValue = 0, lowImageValue = 0;

	public void setHighImagePosition(int highImagePosition) {
		this.highImagePosition = highImagePosition;
	}

	public void setHighImageValue(float highImageValue) {
		this.highImageValue = highImageValue;
	}

	public int getHighImagePosition() {
		return highImagePosition;
	}

	public float getHighImageValue() {
		return highImageValue;
	}

	public void setLowImagePosition(int lowImagePosition) {
		this.lowImagePosition = lowImagePosition;
	}

	public void setLowImageValue(float lowImageValue) {
		this.lowImageValue = lowImageValue;
	}

	public int getLowImagePosition() {
		return lowImagePosition;
	}

	public float getLowImageValue() {
		return lowImageValue;
	}

	public float intersectImageWidth = 0f;

	public float getIntersectImageWidth() {
		return intersectImageWidth;
	}
	...
	protected void calcMinMax(boolean fixedValues) {
		...
		if (!fixedValues) {
			...
			if (mStartAtZero) {
				...
				// mYChartMin = mYChartMin;
				// mYChartMax = mYChartMax;
				...
				if (!compare) {
					if (highImageValue != 0) {
						if (highImageValue > mYChartMax) {
							mYChartMax = highImageValue;
						}
						if (lowImageValue < mYChartMin) {
							mYChartMin = lowImageValue;
						}
					}
				}
				int labelCount = mYLabels.getLabelCount();
				double range = Math.abs(mYChartMax - mYChartMin);
				...
				double intervalMagnitude = Math.pow(10,
						(int) Math.log10(interval));
				int intervalSigDigit = (int) (interval / intervalMagnitude);
				if (intervalSigDigit > 5) {
					// Use one order of magnitude higher, to avoid intervals
					// like 0.9 or
					// 90
				...
				if (interval == 0) {
					...
					interval = Double.valueOf(form.format(rawInterval));
				}
				if (labelCount != 2) {
					mYChartMax += interval;
					mYChartMin -= interval;
	...
	private void drawXLabels() {
		...
		// TOP, BOTTOM, BOTH_SIDED, TOP_INSIDE, BOTTOM_INSIDE, NONE
		if (mXLabels.getPosition() == XLabelPosition.TOP) {
		...
		} else if (mXLabels.getPosition() == XLabelPosition.BOTTOM) { // BOTH
																		// SIDED
	...
	private void drawYLabels(float xPos, float[] positions, float yOffset) {
		...
		for (int i = 0; i < mYLabels.mEntryCount; i++) {
			...
			Paint tempY = new Paint(mYLabelPaint);
			tempY.setColor(Color.TRANSPARENT);
			mDrawCanvas.drawText("2.222m", xPos,
					positions[i * 2 + 1] + yOffset, tempY);
			// mYLabelPaint.setColor(Color.RED);
