 ...
 */
public class OHLCStickChart extends BarLineChartBase<CandleData> {

	public OHLCStickChart(Context context) {
		super(context);
	}

	public OHLCStickChart(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public OHLCStickChart(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void drawData() {
		...
		for (int i = 0; i < mData.getDataSetCount(); i++) {
			...
			for (int j = 0; j < entries.size() * mPhaseX; j++) {
				...
				if (isOffContentLeft(rightBody) && isOffContentTop(low)
				...
				if (open > close) {
					...
					mDrawCanvas.drawLine(xShadow, low, xShadow, high,
							mRenderPaint);
					mRenderPaint.setStyle(Paint.Style.FILL);
					// draw the body
					// mDrawCanvas.drawRect(leftBody, close, rightBody, open,
					// mRenderPaint);
					// mDrawCanvas.drawLine(leftBody, xShadow, close, xShadow,
					// mRenderPaint);
					// leftz, topy,rightx,bottomy
					mDrawCanvas.drawLine(leftBody, close, xShadow, close,
							mRenderPaint);
					mDrawCanvas.drawLine(xShadow, open, rightBody, open,
							mRenderPaint);
					...
					mRenderPaint.setStyle(Paint.Style.FILL);
					// draw the body
					// mDrawCanvas.drawRect(leftBody, open, rightBody, close,
					// mRenderPaint);
					mDrawCanvas.drawLine(leftBody, close, xShadow, close,
							mRenderPaint);
					mDrawCanvas.drawLine(xShadow, open, rightBody, open,
							mRenderPaint);
					// mDrawCanvas.drawLine(xShadow, open, xShadow, close,
					// mRenderPaint);
				}
			}
		}
	 ...
	 * @param bodySpace
	 */
	private void transformBody(float[] bodyPoints, CandleEntry e,
			float bodySpace) {
	...
	protected void drawHighlights() {
		...
		for (int i = 0; i < mIndicesToHightlight.length; i++) {
			...
			CandleDataSet set = mData.getDataSetByIndex(mIndicesToHightlight[i]
					.getDataSetIndex());
			...
			float[] vertPts = new float[] { xIndex, mYChartMax, xIndex,
					mYChartMin, xIndex + 1f, mYChartMax, xIndex + 1f,
					mYChartMin };

			float[] horPts = new float[] { 0, low, mDeltaX, low, 0, high,
					mDeltaX, high };
