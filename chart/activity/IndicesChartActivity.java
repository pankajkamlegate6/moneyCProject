...
public class IndicesChartActivity extends ChartBaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		color = getIntent().getIntExtra("color", R.color.green);
		newBundle = new Bundle();

		newBundle.putString("Id", UrlUtils.indicesChartAPI(mContext, Id,
				getIntent().getStringExtra("range")));
		addGoogleAnaylaticsEvent(AnalyticsTag.MARKETS, AppData.TA_GA,
				AnalyticsTag.INDEX, Id, AnalyticsTag.OVERVIEW,
				AnalyticsTag.CHART);
		IndicesChartFragment fragment = new IndicesChartFragment();
	...
	public void initialActionBarSetUp(StickGraphBean bean) {
		if (!setupActionBar) {
			...
			chartTitle.setText(bean.getGraph().getName());
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		if (!firstLoad) {
			DropDownBean dropDownBean = (DropDownBean) times.getAdapter()
					.getItem(position);
			String uniqueId = dropDownBean.getUniqueId();
			String url = dropDownBean.getUrl();
			updateCharts(url, uniqueId);
	...
	public void updateCharts(String url, String uniqueId) {
		Fragment fragment = getSupportFragmentManager().findFragmentById(
				R.id.chartcontainer);
		if (fragment instanceof IndicesChartFragment) {
			((IndicesChartFragment) fragment).updateChart(url, uniqueId);
		}
	}
