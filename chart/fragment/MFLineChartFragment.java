...
public class MFLineChartFragment extends BaseFragement implements
	...
	public void getGraphView(StickGraphBean gd) {
		...
		if (gd != null) {
			if (isAdded()) {
				MFChartActivity activity = (MFChartActivity) getActivity();
				activity.initialActionBarSetUp(gd.getTabs());
			}
	...
	private void setData(boolean filled) {
		if (times != null) {
			...
			if (getActivity() instanceof MFChartActivity) {
				MFChartActivity activity = (MFChartActivity) getActivity();
				activity.hideVolume();
			}
		...
		if (e instanceof Entry) {
			...
			if (getActivity() instanceof MFChartActivity) {
				...
				if (volumeArray.size() > 0) {
					...
					if (vol != null) {
						...
						vol = Utility.getHtmlStringGraph(getActivity()
								.getResources().getString(R.string.VolChart),
								LargeValueFormatter
				...
				activity.showValue(Utility.getHtmlStringGraph(
						getString(R.string.my_pf_item_nav),
						Utility.threeDecimalFormat(val)), null, Utility
	...
	public void updateChart(String tag) {
		if (times != null) {
			url = tag;
