...
public class BarLineChartTouchListener<T extends BarLineChartBase<? extends BarLineScatterCandleData<? extends BarLineScatterCandleRadarDataSet<? extends Entry>>>>
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mTouchMode == DRAG) {
				...
				if (mChart.isScaleEnabled())
					...
					&& Math.abs(distance(event.getX(), mTouchStartPoint.x,
							event.getY(), mTouchStartPoint.y)) > 25f) {
	...
	private void removeHighlight(MotionEvent event) {
		mChart.highlightTouch(null);
		mLastHighlighted = null;

	}

	public void drawHiglight(MotionEvent e) {
	...
	private void performDrag(MotionEvent event) {
		...
		if (!mChart.isInvertYAxisEnabled()) {
			...
			mMatrix.postTranslate(dragPoint.x - mTouchStartPoint.x,
					-(dragPoint.y - mTouchStartPoint.y));
		}
	}
	...
	public PointF getTrans(float x, float y) {
		...
		// check if axis is inverted
		if (!mChart.isInvertYAxisEnabled()) {
			yTrans = -(mChart.getMeasuredHeight() - y - mChart
					.getOffsetBottom());
		} else {
	...
	public boolean onDoubleTap(MotionEvent e) {
		...
		if (mChart.isDoubleTapToZoomEnabled()) {
			...
			Log.i("BarlineChartTouch", "Double-Tap, Zooming In, x: " + trans.x
					+ ", y: " + trans.y);
		}
