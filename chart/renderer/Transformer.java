...
public class Transformer {
	...
	public void prepareMatrixOffset(ChartInterface chart) {
		...
		if (!mInvertYAxis)
			...
			mMatrixOffset.setTranslate(chart.getOffsetLeft(),
					-chart.getOffsetTop());
	 ...
	 * @return
	 */
	public float[] generateTransformedValuesLineScatter(
			ArrayList<? extends Entry> entries, float phaseY) {
	...
	public float[] generateTransformedValuesBarChart(
			ArrayList<? extends Entry> entries, int dataSet, BarData bd,
			float phaseY) {
	 ...
	 * @return save
	 */
	public synchronized void centerViewPort(final float[] pts,
			final ChartInterface chart) {
	...
	public float[] generateImagePoint(float x, float phaseY, int position) {
		float[] valuePoints = new float[2];
		for (int j = 0; j < valuePoints.length; j += 2) {

			valuePoints[0] = position;
			valuePoints[j + 1] = x * phaseY;
